...
// These tests document performance issues - they may fail until fixes are applied.
// They serve as regression guards after fixes are merged.
//
// Run with: STRESS_TEST=1 go test -v -run Stress -timeout 5m
// Skip in CI by default (no STRESS_TEST env var)

func skipUnlessStressTest(t *testing.T) {
